...
BASE_MODEL_PATH = 'intfloat/e5-base-v2'

TEST_DATA_PATH = './test_gpt_verified.json'
TRAIN_DATA_PATH = './train_gpt15k_verified.json'
...
def train_MCQA(
    ...
    """
    Launch training for MCQA model. 
    """

    model, tokenizer = load_model(BASE_MODEL_PATH)
    data_collator = MCQACollator(tokenizer=tokenizer)

    wandb.init(project="310-project-runs")
    os.environ["WANDB_PROJECT"] = "310-project-runs"
    os.environ["WANDB_LOG_MODEL"] = "true"
    os.environ["WANDB_WATCH"] = "false"

    global TEMPERATURE_QA
    TEMPERATURE_QA = temperature_qa
    
    global TEMPERATURE_EXP
    ...
    training_args = TrainingArguments(
        ...
        report_to="wandb",
        logging_steps=loss_steps,
        #eval_steps=loss_steps,
...
if __name__ == "__main__":
    ...
    # log in to wandb to track progress
    wandb.login()

    np.random.seed(2345)
    values = np.arange(0.05, 1.05, 0.05)
    grid = np.array([(a, b) for a, b in itertools.product(values, values)])
    temps = grid[np.random.choice(grid.shape[0], 12, replace=False)]


    for temp_qa, temp_exp in temps:
        
        train_MCQA(
                train_dataset=dataset_train,
                eval_dataset=dataset_eval,
                path_to_dir=f'./run_{temp_qa}_{temp_exp}',
                batch_size=5,
                loss_steps=100,
                learning_rate=1e-5, 
                temperature_qa=temp_qa,
