...
from collator import MCQACollator
...
class MCQATrainer(Trainer):
    def compute_loss(self, model, inputs):
        ...
        loss = []model, tokenizer = load_model('./run1/checkpoint-4000')
        terms = 0
        for (
            embedding,
            pos_spans,
            ...
            if len(pos_answer_embeddings) != 0 and len(neg_answer_embeddings) != 0:
                ...
                key_answer_loss = InfoNCE(temperature=TEMPERATURE, negative_mode='paired', reduction='none')(
            ...
            if len(pos_span_embeddings) != 0 and len(neg_span_embeddings) != 0:
                ...
                for pos_answer_embedding in pos_answer_embeddings:
                    for pos_span_embedding in pos_span_embeddings_joined:
                        loss += [InfoNCE(temperature=TEMPERATURE, negative_mode='paired', reduction='none')(
